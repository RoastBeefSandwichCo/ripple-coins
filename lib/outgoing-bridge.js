...
var coinProcessing = {
    ...
    validateAddress : function (thisWithdrawal, fnClearPending, callback){
        ...
        this.coinDaemons[currency].validateAddress(address, function(err, isValid, resHeaders){ //watch for problems caused by order of arguments
console.log('valStatus',this.valStatus);
/*            if (err) {
...
*/
	    ...
	    if (isValid.isvalid){
                //console.log(logPrefix, '>VALIDATED<:', currency, isValid.address,')');
				console.log(logPrefix, '>VALIDATED<:', this.currMap[isValid.address], isValid.address);
                valStatus = true;
				callback(valStatus, this.currMap[isValid.address], isValid.address);
				//delete
				return true;
            }else{
